...
// DownloadHandler accepts POST uploaded files for user jobs
func (hc HTTPConnector) DownloadHandler(w http.ResponseWriter, r *http.Request) {
	/*
		...
		isHost := false
		for _, h := range hc.hMgr.GetHosts() {
			if h.Address == ip {
				isHost = true
