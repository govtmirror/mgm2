...
func (db db) ScrubPasswordToken(token uuid.UUID) error {
...
func (db db) ExpirePasswordTokens() error {
	con, err := sql.Open("mysql", fmt.Sprintf("%v:%v@tcp(%v:3306)/%v", db.user, db.password, db.host, db.database))
	if err != nil {
		return err
	}
...
func (db db) CreatePasswordResetToken(userID uuid.UUID) (uuid.UUID, error) {
	con, err := sql.Open("mysql", fmt.Sprintf("%v:%v@tcp(%v:3306)/%v", db.user, db.password, db.host, db.database))
	if err != nil {
...
func (db db) ValidatePasswordToken(userID uuid.UUID, token uuid.UUID) (bool, error) {
	con, err := sql.Open("mysql", fmt.Sprintf("%v:%v@tcp(%v:3306)/%v", db.user, db.password, db.host, db.database))
	if err != nil {
		return false, err
	}
